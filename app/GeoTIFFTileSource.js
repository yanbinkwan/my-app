...
export default function ($) {
    ...
    if (input.GeoTIFF && input.GeoTIFFImages) {
      ...
      this.promises = {
        GeoTIFF:
          input instanceof File
            ? GeoTIFF.fromBlob(input)
            : GeoTIFF.fromUrl(input),
      ...
      this.promises.GeoTIFF.then(tiff => {
        ...
        .then(count => {
          ...
          let promises = [...Array(count).keys()].map(index =>
            self.GeoTIFF.getImage(index)
          );
  ...
  $.GeoTIFFTileSource.getAllTileSources = async function (input, opts) {
    let tiff =
      input instanceof File ? GeoTIFF.fromBlob(input) : GeoTIFF.fromUrl(input);
    return tiff
      .then(t => {
        tiff = t;
      ...
      .then(c =>
        Promise.all([...Array(c).keys()].map(index => tiff.getImage(index)))
      )
      .then(images => {
        ...
        let tilesources = imagesets.map(
          images =>
            new $.GeoTIFFTileSource(
              { GeoTIFF: tiff, GeoTIFFImages: images },
              opts
            )
        );
        return tilesources;
  ...
  $.extend(
    ...
    /** @lends OpenSeadragon.GeoTIFFTileSource.prototype */ {
      /**
      ...
      getTileUrl: function (levelnum, x, y) {
        ...
        url.fetch = (
          (ts, level, x, y, src) => () =>
            regionToDataUrl.call(ts, level, x, y, src)
        )(this, level, x, y, url);
  ...
  function regionToDataUrl(level, x, y, src) {
    ...
    return level.image
      .getTileOrStrip(x, y, null, this._pool, abortSignal)
      .then(raster => {
